...
```{r}
#| include: false
params <- list(cycle = "2023H",
               nummer_i_rekken = "trettien",
               gjennomforing = "høsten 2023",
               datefrom = "21. september",
               dateto = "30. oktober")
kapittel_mappe <- "02_Beskrivelse_av_utvalg_og_gjennomforing"

source("../../general_formatting.R")
source(file.path(kapittel_mappe, "2_0_stibaner.R"))
source(file.path(kapittel_mappe, "2_1_funksjoner.R"))
source(file.path(kapittel_mappe, "2_2_tabeller.R"))
source(file.path(kapittel_mappe, "2_3_kart.R"))
source(file.path(kapittel_mappe, "2_4_tekstbiter.R"))
```
...
I @tbl-2.6 og @tbl-2.7 vises svarprosent for grunnskolene sortert på henholdsvis landsdel og skoletype, og landsdel og skolestørrelse. @tbl-2.6 viser at svarprosenten totalt sett er relativ lik mellom barneskoler, 1--10-skoler og ungdomsskoler ([`r paste0( range(as.matrix(tbl_2.6[tbl_2.6$Landsdel == "Totalt", -c(1,4)]), na.rm=TRUE), collapse='-')`]). Når det gjelder landsdel totalt er svarprosenten høyest blant `r ranger2(tbl_2.6, c("Barneskole", "Ungdomsskole", "1-10 skole"), "Landsdel")$høy`. Svarprosenten er klart lavest blant `r ranger2(tbl_2.6, c("Barneskole", "Ungdomsskole", "1-10 skole"), "Landsdel")$lav`.
...
@tbl-2.8 viser at det generelt er små forskjeller mellom nettoutvalget og populasjonen når vi ser på totalen for den enkelte skoletype. Forskjellen er på mindre enn 1 prosentpoeng for alle skoletyper. Ser vi på landsdel er også forskjellene generelt små. Det er en liten underrepresentasjon av grunnskoler fra Midt- og Nord-Norge på `r abs(min(tbl_2.8[["_data"]][["Totalt_Utvalg %"]]-tbl_2.8[["_data"]][["Totalt_Pop. %"]]))` prosentpoeng. Det er en liten overrepresentasjon av grunnskoler fra Oslo-området på `r abs(max(tbl_2.8[["_data"]][["Totalt_Utvalg %"]]-tbl_2.8[["_data"]][["Totalt_Pop. %"]]))`. Alt i alt er det svært små forskjeller innad i hver landsdel mellom utvalg og populasjon.
...
Når det gjelder skolestørrelse viser @tbl-2.9 at deltakelsen er svært nær det vi finner i populasjonen. Det største avviket vi finner er for skoler med mindre enn 100 elever, som har en underrepresentasjon på 4,2 prosentpoeng.
...
Svarprosenten for de videregående skolene varierer betydelig mellom fylkene (se også @fig-2_3). Samtlige fylker, med unntak av Innlandet, har en deltakelse på minst `r min(tbl_2.11$Svarprosent)` prosent. `r if(min(tbl_2.11$Svarprosent)>50) "Dette er høyt." else "Dette er akseptabelt."` Lavest svarprosent fant vi i `r ranger2(tbl_2.11, "Svarprosent", "Fylke")$lav`, mens den var høyest i `r ranger2(tbl_2.11, "Svarprosent", "Fylke")$høy`.
...
@tbl-2.12 viser at forskjellene mellom populasjon og utvalg for de tre ulike typene videregående skoler er liten. Det samme gjelder landsdel der forskjellene kun er innenfor et prosentpoeng.
...
@tbl-2.13 viser at det er små avvik mellom utvalg og populasjon når det gjelder skolestørrelse. Skolene med mellom 250 og 599 elever er svakt overrepresentert, mens skolene med mer enn 600 elever svakt underrepresentert. Ser vi innad i landsdelene er imidlertid ikke avvikene særlig store.
...
@tbl-2.14 indikerer at fordelingen i nettoutvalget når det gjelder offentlig og privat eierskap korresponderer ganske godt med fordelingen i populasjonen. Vi finner relativt små forskjeller i representativitet mellom offentlige og private, og mellom bokmålsskoler og nynorskskoler.
...
@tbl-2.15 viser hvordan utvalget var sammensatt etter fylke, populasjon, bruttoutvalg og hvilken svarprosent som ble oppnådd. I nesten samtlige fylker har over 50 prosent av de inviterte kommunene svart. `r ranger2(tbl_2.15, "Svarprosent", "Fylke")$lav` skiller seg ut med en deltakelse på under 50 prosentpoeng (se også @fig-2_4).
...
@tbl-2.16 viser at svarprosenten varierer noe etter kommunestørrelse og landsdel, med unntak av `r ranger2(tbl_2.16, c("Under 5000", "5000 til 19 999", "Mer enn 20 000"), "Landsdel")$høy` som har en klart høyere deltakelse. Innad i landsdelen er det særlig de minste kommunene i Midt- og Nord-Norge og Sør- og Vest-Norge som peker seg ut med særlig lav deltakelse, fulgt av de største kommunene i Midt- og Nord-Norge.

<!-- og `r ranger2(tbl_2.16, c("Under 5000", "5000 til 19 999", "Mer enn 20 000"), "Landsdel")$lav` (Oslo-området har ingen kommuner under 5000 innbyggere).  -->
...
Spørringene ble gjennomført i perioden `r params$datefrom` til `r params$dateto`. Undersøkelsen ble distribuert elektronisk til alle de fire målgruppene, og det ble gitt i alt fire purringer, omtrent én per uke. Vi har ikke fått henvendelser om tekniske problemer i forbindelse med gjennomføringen.
...
```{r}
#| tbl-cap: '_Hvem svarer på vegne av skolen? Flere kryss mulig_'
#| label: tbl-2.18
tbl_2.18
```
